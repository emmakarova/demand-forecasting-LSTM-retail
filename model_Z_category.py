...
DATA_PATH = "~/магистър ИИБФ/дипломна/data/model/processed/daily_sales_15875_with_weather.csv"
BARCODE = "15875" # 15875 11180 7590 6362 6371
RANDOM_SEED = 42
...
# 1. Изчисляваме нетната промяна за деня (това си остава същото)
df['stock_change'] = df['Delivery_QTTY'] - df['Sales_QTTY']

# 2. НОВА ЛОГИКА: Итеративно изчисляване със "Зануляване" (Floor Clamping)
recalculated_stock = []
current_val = 0 # Начална наличност (може да се промени, ако знаете стартовата)

for i in range(len(df)):
    # Взимаме промяната за конкретния ден
    change = df['stock_change'].iloc[i]
    
    # Добавяме я към текущия склад
    current_val += change
    
    # ПРАВИЛОТО: Складът не може да е отрицателен!
    # Ако стане < 0, приемаме, че е 0 (все едно е имало невидима доставка, която е покрила разликата)
    if current_val < 0:
        current_val = 0
        
    recalculated_stock.append(current_val)

# 3. Записваме новата логична наличност в колоната
df['stock'] = recalculated_stock

# 4. Проверка на резултата (за същия проблемен период)
print("--- СЛЕД КОРЕКЦИЯТА (НЯМА ОТРИЦАТЕЛНИ СТОЙНОСТИ) ---")
print(df[['Delivery_QTTY', 'Sales_QTTY', 'stock', 'stock_change']].iloc[1100:1151])
...
log_path = "~/магистър ИИБФ/дипломна/data/model/processed/lstm_log_entries_Z.csv"
